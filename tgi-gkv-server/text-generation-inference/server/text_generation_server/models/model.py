...
from text_generation_server.gkv_cache.gkv_cache import GKVCacheEntry, GKVCacheManager
...
class Model(ABC):
    def __init__(
        ...
        **kwargs
        ...
        ## GKV Cache Manager Support
        self.gkv_manager = GKVCacheManager(
            max_entries=kwargs.get("max_gkv_cache_entries", 100),
            ttl=kwargs.get("ttl", 9999)
        )
        ##
        self.check_initialized()


    ### GKV Cache Handling Aliases Methods
    def get_gkv_entry(self, cache_id: str) -> GKVCacheEntry:
        return self.gkv_manager.get(cache_id)

    def store_gkv_entry(self, req_id: str, prefix_pos: int, past_key_values) -> str:
        gkv_data = past_key_values
        # may need to format data TODO
        return self.gkv_manager.store(req_id, prefix_pos, gkv_data)
    ### 
