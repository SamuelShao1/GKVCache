...
class CausalLM(Model):
    ...
    def generate_token(
        ...
        gkv_cache_id = None
        if batch.requests and hasattr(batch.requests[0], "gkv_cache_id") and batch.requests[0].gkv_cache_id:
            gkv_cache_id = batch.requests[0].gkv_cache_id
            cache_entry = self.get_gkv_entry(gkv_cache_id)

            if cache_entry:

                batch.past_key_values = cache_entry.past_kv_data

                print(f"GKV Cache - Token generation using cache entry for id: {gkv_cache_id} at prefix position {cache_entry.prefix_pos}")
        ...
        ### GKV Caching Logic - Save to Cache
        if not gkv_cache_id and batch.requests and batch.requests[0].id:
            print("GKV Cache - Saving cache entry for id: ", batch.requests[0].id)
            prefix_pos = batch.input_lengths[0] if batch.input_lengths else len(batch.all_input_ids[0])

            new_cache_entry_id = self.store_gkv_entry(req_id=batch.requests[0].id, prefix_pos=prefix_pos, past_kv_data=past)

            for generation in generations:
                if not hasattr(generation, "metadata"):
                    generation.metadata = {}
                generation.metadata["gkv_cache_id"] = new_cache_entry_id
        ### 
