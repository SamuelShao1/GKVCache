...
class TextGenerationService(generate_pb2_grpc.TextGenerationServiceServicer):
    ...
    async def GetGKVEntry(self, request, context):

        cache_id = request.cache_id
        entry = self.model.get_gkv_entry(cache_id)
        if not entry:
            return generate_pb2.GetGKVEntryResponse(
                found=False
            )
        ...
        return generate_pb2.GetGKVEntryResponse(
            found = True,
            req_id = entry.req_id,
            prefix_pos = entry.prefix_pos,
        )
    
    async def ClearGKVCache(self, request, context):
        self.model.gkv_manager.clear()
        return generate_pb2.ClearGKVCacheResponse(
            cleared = True
        )

    async def GetGKVCacheStatistics(self, request, context):
        stats = self.model.gkv_manager.get_stats()
        return generate_pb2.GetGKVCacheStatisticsResponse(
...
def serve(
    ...
    async def serve_inner(
        ...
        try:
            ...
            logger.info("="*50)
            logger.info(f"MODEL CLASS USED: {model.__class__.__module__}.{model.__class__.__name__}")
            logger.info(f"MODEL TYPE: {type(model)}")
            logger.info(f"MODEL ATTRIBUTES: {dir(model)[:10]}...")
        ...
        logger.info("SAMMMMMMM Server started at {}".format(local_url))
